...
public class TeamFormationCodification extends Codification {
    ...
    private List<Worker> searchArea;
    ...
    public TeamFormationCodification(List<Constrain> restrictions, TeamFormationProblem problem, List<Worker> searchArea) {
        super();
    ...
    public Double getPenalEvaluation() {
        return penalEvaluation;
    }

    public void setPenalEvaluation(Double penalEvaluation) {
        this.penalEvaluation = penalEvaluation;
    }

    public List<Worker> getSearchArea() {
        return searchArea;
    }

    public void setSearchArea(List<Worker> searchArea) {
        this.searchArea = searchArea;
    }

    public TeamFormationCodification() {
        super();

    }

    public TeamFormationProblem getProblem() {
        return problem;
    }

    public void setProblem(TeamFormationProblem problem) {
        this.problem = problem;
    }

    public List<Constrain> getRestrictions() {
        return restrictions;
    }

    public void setRestrictions(List<Constrain> restrictions) {
        this.restrictions = restrictions;
    }
    ...
    public Object getVariableAleatoryValue(int variable) {
        ...
        while (iteration < this.count_iteration_aleatory_search && !found) {
            if (roleWorker.getFixedWorkers().size() != roleWorker.getWorkers().size() && !roleWorker.getWorkers().isEmpty()) {//Significa que el rol no est� fijo
                ...
                Worker newWorker = searchArea.toArray(new Worker[0])[pos];
    ...
    public static List<Worker> actualiceTeam(State state, int i) {
    ...
    public boolean checkTeamRestrictions(State state) {
        ...
        while (i < restrictions.size() && meet) {
            ...
            if (rest instanceof DiferentPersonByProject) {
                meet = rest.ValidateState(state);
            }
    ...
    public boolean validatingTeamRestrictions(State state) throws IOException, ClassNotFoundException {
        ...
        while (i < restrictions.size()) {
            ...
            if (rest instanceof MinimumRoles) {
                ...
                meet = rest.ValidateState(state);
                if (meet) {
                    ((MinimumRoles) rest).invalidateState(state);
                }
                meet = rest.ValidateState(state);
                if (!meet) {
                    repareTeamRestrictionsState(state);
    ...
    public void repareIndividualRestrictionsState(State state) {
        ...
        ArrayList<Worker> peopleSol = new ArrayList<>(); //lista de todas las presonas de la solucion
        ArrayList<Worker> peopleSol2 = new ArrayList<>(); //lista de todas las presonas de la solucion asignadas antes de un rol y proyecto especifico
        List<Worker> peopleNoJP = new ArrayList<>(); // lista para añadir a todas las personas de la solucion que no son jefes de proyecto
        List<Worker> peopleProyect = new ArrayList<>();  //lista de personas por proyecto
                        ...
                        if (roleWorker.getRole().isBoss()) { // si el rol actual es jefe de proyecto
                            if (rest instanceof IsEJ) {
                                ...
                                while (!meet && in < cantIntentos) {
                                    Worker workerChange = changeInvalidPersonSolution(state, worker, i, j);
    ...
    public Worker changeInvalidPersonSolution(State state, Worker invalidWorker, int i, int j) {
        ArrayList<Object> projects = state.getCode();
    ...
    public void validatingIndividualRestriccion(State solution) throws IOException, ClassNotFoundException {
        ...
        Worker worker = roleWorker.getWorkers().get(0);
        ...
        while (i < restrictions.size()) {
            ...
            if (rest instanceof IsEJ) {
                meet = rest.ValidateState(solution);
                if (meet) {
                    ((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(0).getRole().setBoss(true);

                    ((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(0).getWorkers().get(0).getWorkerTest().setTipoMB("xxxx");
                }
                meet = rest.ValidateState(solution);
                if (!meet) {
            ...
            if (rest instanceof IsISCO) {
                ...
                if (meet) {
                    ...
                    ((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(0).getWorkers().get(0).getWorkerTest().setIS('I');

                    ((ProjectRole) solution.getCode().get(0)).getRoleWorkers().get(0).getWorkers().get(0).getWorkerTest().setCO('I');
                }
                meet = rest.ValidateState(solution);
                if (!meet) {
                    repareIndividualRestrictionsState(solution);
            ...
            if (rest instanceof IsProjectBoss) {
                ...
                if (meet) {
                    //((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(0).getRole().setRoleName("Jefe de Proyecto");
                    //se asigna la persona que está en el primer proyecto con rol de Jefe al segundo proyecto con el mismo rol
                    Worker workerC = ((ProjectRole) solution.getCode().get(0)).getRoleWorkers().get(0).getWorkers().get(0);
            ...
            if (rest instanceof IsWorkerAssigned) {
                ...
                if (meet) {
                    //((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(0).getRole().setRoleName("Jefe de Proyecto");
                    //((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(1).getRole().setRoleName("Programador");
                    //se asigna la persona q está en el rol Jefe de Proyecto al rol programador y asi esta en ambos roles
                    Worker workerI = ((ProjectRole) solution.getCode().get(1)).getRoleWorkers().get(0).getWorkers().get(0);
            ...
            if (rest instanceof AllCompetitionLevels) {
                ...
                if (meet) {
                    ...
                    ((ProjectRole) solution.getCode().get(0)).getRoleWorkers().get(0).getWorkers().get(0).getCompetenceValueList().get(0).getLevelFk().setLevels(-1);
                }
                meet = rest.ValidateState(solution);
                if (!meet) {
                    repareIndividualRestrictionsState(solution);
            ...
            if (rest instanceof MaximumRoles) {
                ...
                if (meet) {
                    ...
                    ((ProjectRole) solution.getCode().get(0)).getRoleWorkers().get(1).getWorkers().set(0, worker);
                }
                meet = rest.ValidateState(solution);
                if (!meet) {
                    repareIndividualRestrictionsState(solution);
            ...
            if (rest instanceof WorkerRemovedFromRole) {
                ...
                if (meet) {
                    ...
                    assignedRole.setRolesFk(roleWorker.getRole());
            ...
            if (rest instanceof MaxWorkload) {
                ...
                if (meet) {
                    ...
                    ((ProjectRole) solution.getCode().get(0)).getRoleWorkers().get(0).getWorkers().get(0).setWorkload(maxWorkload + rolLoad);
                }
                if (!meet) {
                    repareIndividualRestrictionsState(solution);
                }
    ...
    public void repareTeamRestrictionsState(State state) throws IOException, ClassNotFoundException {
        ...
        while (i < projects.size()) { //para cada projecto-rol
            ...
            while (a < restrictions.size()) {
                ...
                if (rest instanceof DiferentPersonByProject) {
                    meet = ((DiferentPersonByProject) rest).validateProyect(state, i);
                    while (!meet && in < cantIntentos) {
                        ((DiferentPersonByProject) rest).repareState(state, i);
                        meet = ((DiferentPersonByProject) rest).validateProyect(state, i);
                        in++;
                    }
                }
    ...
    public int contarCerebros(State state) {
        ...
        for (Object project : projects) { //para cada projecto-rol
            ...
            for (RoleWorker roleWorker : roleWorkers) { //para cada rol-persona
                ...
                aux.addAll(roleWorker.getWorkers());
                aux.addAll(roleWorker.getFixedWorkers());
                for (Worker worker : aux) { // para cada persona
                    if (worker.getWorkerTest() != null) {
                        if (worker.getWorkerTest().getCE() != 'I' && worker.getWorkerTest().getCE() != 'E') { // si tiene rol cerebro
                            personCount++; //cuento uno
                        }
